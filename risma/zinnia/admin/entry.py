...
class ContenuMedia(admin.StackedInline):
    extra = 1
    model = ContenuMedia
    fk_name = "entry"
    verbose_name = "Médias supplémentaire"  
    
    
class MediaPrincipal1(ContenuMedia):
        max_num = 1
        fk_name = "media_principal1"
        verbose_name = "Media principal 1"
        exclude = ('integration',)
        
        def queryset(self, request):
            qs = super(MediaPrincipal1, self).queryset(request).filter(media_principal1__isnull=False)
            return qs
                       
                                
class MediaPrincipal2(MediaPrincipal1):
        fk_name = "media_principal2"
        verbose_name = "Media principal 2"            
                
        def queryset(self, request):
            qs = super(MediaPrincipal1, self).queryset(request).filter(media_principal2__isnull=False)
            return qs
...
class EntryAdmin(admin.ModelAdmin):
    ...
    fieldsets = ((_('Content'), {'fields': ('title', 'content', 'status')}),
                 (_('Options'), {'fields': ('featured', 'excerpt', 'related',
    ...
    list_display = ('get_title', 'get_authors', 'get_categories',
                    'get_tags', 'get_comments_are_open',
                    'pingback_enabled', 'get_is_actual',
                     'get_is_visible', 'get_link', 'creation_date')
    ...
    inlines = [MediaPrincipal1, MediaPrincipal2, ContenuMedia]
    ...
    def _media(self):
        STATIC_URL = '%szinnia/' % project_settings.MEDIA_URL
        ...
        """
        media += Media(
            js=('%sckeditor/jquery.markitup.js' % STATIC_URL,
                '%sjs/markitup/sets/%s/set.js' % (
                    STATIC_URL, settings.MARKUP_LANGUAGE),
                reverse('admin:zinnia_entry_markitup')),
            css={'all': (

...
@register.inclusion_tag('zinnia/tags/dummy.html')
def get_most_commented(number=5, template='zinnia/tags/popular_entries.html'):
    stats = EntryNombreLus.objects.filter(
                    date_consultation__range=(datetime.now() - timedelta(days=7), datetime.now())).values("entry").annotate(
                    nombre_visites=Count('entry')).order_by('-nombre_visites')[:number]
    object_dict = Entry.published.in_bulk([stat['entry'] for stat in stats])

    return {'template': template,
            'entries': object_dict.values()}